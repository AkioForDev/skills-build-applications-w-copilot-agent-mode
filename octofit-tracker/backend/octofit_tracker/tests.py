...
class TeamModelTest(TestCase):
    def test_create_team(self):
        user = User.objects.create(username="teamuser", email="team@example.com", password="password123")
        team = Team.objects.create(name="Test Team", members=[user])
        self.assertEqual(team.name, "Test Team")

class ActivityModelTest(TestCase):
    def test_create_activity(self):
        user = User.objects.create(username="activityuser", email="activity@example.com", password="password123")
